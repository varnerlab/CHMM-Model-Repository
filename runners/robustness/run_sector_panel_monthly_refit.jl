# =========================================================================== #
# run_sector_panel_monthly_refit.jl
#
# Peer-review item P5.2 (R1.Exp4): monthly-refit version of the 30-ticker
# sector-balanced cross-ticker panel, parallel to run_sector_panel_monthly_refit.jl.
# Same protocol (penalised CHMM-t at K=18, λ=20, N_paths=1000) but the model is
# refit every 21 trading days (~one calendar month) instead of 63 (~one quarter).
# The monthly OoS path is the concatenation of per-month simulations under
# the refit-at-month-start parameters.
#
# Output:
#   results/sector_panel/sector_panel_monthly_refit.{csv,txt}
# =========================================================================== #
...
const REFIT_CADENCE = 21;     # monthly (~21 trading days per calendar month)
...
println("="^80)
println("  monthly-refit cross-ticker panel (peer-review item 5)")
...
# Build a monthly-refit OoS simulation: at each refit point j, fit on the rolling
...
function _monthly_refit_oos_paths(R_full::AbstractVector, n_is::Int, n_oos::Int,
...
for (sector, tickers) in SECTOR_PANEL
    ...
    for tk in tickers
        ...
        # OoS evaluation under monthly refit
        sim_oos, refits = _monthly_refit_oos_paths(R_full, n_is, n_oos, K_MAIN, N_PATHS;
...
open(joinpath(OUT_DIR, "sector_panel_monthly_refit.csv"), "w") do io
...
open(joinpath(OUT_DIR, "sector_panel_monthly_refit.txt"), "w") do io
    println(io, "="^104);
    println(io, "monthly-refit 30-ticker cross-ticker panel (penalised CHMM-t, K=$K_MAIN, λ=$SHRINK_RATE)");
...
println("[done] $OUT_DIR/sector_panel_monthly_refit.{csv,txt}");

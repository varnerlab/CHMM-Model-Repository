...
| `runners/spectral/run_hmm_acf_capacity.jl` | `results/diagnostics/hmm_acf_capacity.txt` + `.csv` + `results/hmm_acf_capacity/*.jld2` | Appendix `sec:spectral_rank_xticker` (realizable ACF-targeted valid-HMM optimization at K = 3 and K = 18; per-model reload-verified certificates; achieved accuracy under the declared optimizer) |
| `runners/spectral/run_hmm_acf_frontier.jl` | `results/diagnostics/hmm_acf_frontier.txt` + `.csv` + `hmm_acf_frontier_regret.csv` + `results/hmm_acf_frontier/*.jld2` | Appendix `sec:spectral_rank_xticker` (marginal-vs-ACF Pareto frontier at K = 3; weighted-objective sweep, per-ticker joint regrets, nondominated sets, `ml_multi` published-multistart comparator; `--summary-only` rebuilds txt + regret CSV from the main CSV) |

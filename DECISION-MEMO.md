...
| # | Work item                                                                                                                                                                                                                    | Status | Notes                                                                                                                                         |
|---|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------|-----------------------------------------------------------------------------------------------------------------------------------------------|
| V1 | Re-derive DiscreteNJ / WJ $(K_{\text{disc}}, \lambda, \epsilon)$ from `arxiv.org/pdf/2603.10202` (`alswaidan2026hybrid`), re-run `run_baselines_and_cross_asset.jl`, propagate into `methods_v10.tex` and every downstream cell | OPEN   | Current values are $K_{\text{disc}} = 13, \lambda = 3, \epsilon = 0.01$ (`methods_v10.tex:117`); user flagged as too-few-states / weak        |
| V2 | Add axis labels to Fig 4, Fig 5, and any other v10 figure missing them; audit generator scripts                                                                                                                               | OPEN   | Mechanical sweep in `run_figures.jl`, `run_all_analysis.jl`, `run_diagnostics.jl`                                                             |
| V3 | Align the two panels of Fig 4 as a single subplot grid, not two stitched standalone figures                                                                                                                                   | OPEN   | Regenerate from source                                                                                                                        |
| V4 | Rewrite figure and table captions in v10 to be self-contained and informative                                                                                                                                                 | OPEN   | Top-venue style: what the panel shows, what to look for, what the headline finding is                                                         |
| V5 | Remove stale "v6 OoS window" references ($T_{\text{OoS}} = 219$) from v10 section files; check for similar stale-version nuances                                                                                              | OPEN   | Currently at `results_v10.tex:92, 107, 273`                                                                                                   |
| V6 | Consolidate the random seed (20260422) mention to a single body sentence; drop repeats                                                                                                                                        | OPEN   | Best placement: Evaluation Protocol or Data subsection                                                                                        |
| V7 | Fix margin overflow on the §4.6 sig-MMD path-lift sentence `We lift each window to a 2D path (t/W, r_t/std(r))`                                                                                                               | OPEN   | Convert inline math to a displayed equation or rewrap prose to `\textwidth`                                                                   |
| V8 | Fix margin overflow on the leverage-effect sentence `The observed leverage effect on the IS window, measured as avg_{k=1}^{20} corr(r^2_t, r_{t-k})`                                                                          | OPEN   | Same fix as V7                                                                                                                                |
| V9 | Trim unnecessary "reproduced from \ldots" statements in table captions                                                                                                                                                        | OPEN   | Audit `tab:*` captions in `results_v10.tex` / `methods_v10.tex`                                                                               |
| V10 | Reconcile §4.3 title ("seven-model comparison", `results_v10.tex:80`) with the actual generator count in Table~\ref{tab:model_comparison}                                                                                    | OPEN   | User counted 8, not 7                                                                                                                         |
| V11 | Reconcile Table 3 caption ("comparison to 9 generators") with the actual row count and surrounding prose                                                                                                                     | OPEN   | Single authoritative number across caption and body                                                                                           |
| V12 | Add one or two schematic figures (TikZ / SVG) diagramming (i) Pipelines A and B and (ii) the architectures of the generator families                                                                                          | OPEN   | Place at the top of Methods or between Intro and Methods                                                                                      |
| V13 | General figure-quality pass: colorblind-safe palette (max ~6 colors), consistent font / line-width / size, higher-contrast density fans; push defaults into `run_figures.jl`                                                  | OPEN   | Regenerate all `figs/*.svg` after defaults change                                                                                              |
| V14 | Fix Fig 5 panel (b): raise contrast on simulated-path overlay vs observed, add a legend for "Observed" vs "Simulated"; audit other overlay panels                                                                             | OPEN   | Same issue may appear on Pipeline A / B density fans and cross-asset correlation heatmaps                                                     |
| V15 | Final cross-section nuance sweep: scan all v10 section files for stale version references, margin overflows, unlabeled axes, misleading titles, thin captions                                                                 | OPEN   | Do last, after V1 through V14                                                                                                                  |

...
# ========================================================================================= #
# Forward Filter + h-step Regime Forecast (Phase 1 of VIX-CHMM forecasting plan)
# ========================================================================================= #

"""
    _transition_matrix(chmm) -> Matrix{Float64}

Extract the K × K transition matrix from a CHMM where T[i, j] = P(S_{t+1} = j | S_t = i).
"""
function _transition_matrix(chmm::_ContinuousCHMM)::Matrix{Float64}
    K = length(chmm.states);
    T_mat = zeros(K, K);
    for i in 1:K
        T_mat[i, :] = probs(chmm.transition[i]);
...
"""
    forward_filter(chmm, observations; pi_init=nothing) -> Matrix{Float64}

Runs the forward filter and returns the `N × K` matrix of normalized state
posteriors `π_t(s) = P(S_t = s | y_{1:t})`. Uses log-space `_logsumexp_vec`
for numerical stability. If `pi_init` is `nothing`, the stationary
distribution of the transition matrix is used.
"""
function forward_filter(chmm::_ContinuousCHMM, observations::Vector{Float64};
    pi_init::Union{Nothing,Vector{Float64}}=nothing)::Matrix{Float64}

    N = length(observations);
    K = length(chmm.states);
    T_mat = _transition_matrix(chmm);

    π0 = pi_init === nothing ? probs(_stationary_distribution(chmm)) : pi_init;
    π0 = max.(π0, 1e-12);
    π0 ./= sum(π0);

    log_B = zeros(N, K);
    for t in 1:N, k in 1:K
        log_B[t, k] = logpdf(chmm.emission[k], observations[t]);
    end

    log_alpha = zeros(N, K);
    log_alpha[1, :] = log.(π0) .+ log_B[1, :];
    for t in 2:N
        for j in 1:K
            log_alpha[t, j] = _logsumexp_vec(log_alpha[t-1, :] .+ log.(T_mat[:, j])) + log_B[t, j];
        end
    end

    π = zeros(N, K);
    for t in 1:N
        π[t, :] = exp.(log_alpha[t, :] .- _logsumexp_vec(log_alpha[t, :]));
    end
    return π;
end

"""
    regime_forecast(chmm, pi_t, h) -> Matrix{Float64}

Computes the `h`-step-ahead regime-forecast posteriors `π_{t+i|t}` for
`i = 1, ..., h` by matrix-powering the transition operator. Returns an
`h × K` matrix whose `i`th row is `π_{t+i|t}`.

Uses the identity `π_{t+1} = A^⊤ π_t` where `A[i, j] = P(S_{t+1} = j | S_t = i)`.
"""
function regime_forecast(chmm::_ContinuousCHMM, pi_t::Vector{Float64}, h::Int)::Matrix{Float64}
    @assert h >= 1 "h must be >= 1";
    K = length(chmm.states);
    @assert length(pi_t) == K "pi_t must have length K";
    T_mat = _transition_matrix(chmm);

    out = zeros(h, K);
    current = copy(pi_t);
    for step in 1:h
        current = T_mat' * current;
        current ./= sum(current);
        out[step, :] = current;
    end
    return out;

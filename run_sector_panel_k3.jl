# =========================================================================== #
# run_sector_panel_k3.jl
#
# Cross-ticker rebuild at the new body headline operating point K* = 3 (the
# state-resolution-robust held-out-clean default per peer-review R1 W1's
# contingency under R1 RE1's k-fold CV). Same protocol as run_sector_panel_k6.jl
# otherwise: penalised CHMM-t, λ = 20, N_paths = 1000, seed = 20260420.
#
# Output:
#   results/sector_panel/sector_panel_summary_k3.csv
#   results/sector_panel/sector_panel_summary_k3.txt
# =========================================================================== #
...
println("="^80)
println("  Sector-balanced 30-ticker panel at K* = 3 (R1 W1 body-rebuild contingency)")
...
# --- Output ---
open(joinpath(OUT_DIR, "sector_panel_summary_k3.csv"), "w") do io
...
open(joinpath(OUT_DIR, "sector_panel_summary_k3.txt"), "w") do io
    println(io, "="^96);
    println(io, "Sector-balanced 30-ticker panel (penalised CHMM-t, K = $K_MAIN, λ = $SHRINK_RATE)");
    println(io, "[cross-ticker rebuild at the new body headline K* = 3 (R1 W1 contingency)]");
    println(io, "="^96);
    ...
    println(io, "Compare to the K* = 6 panel (results/sector_panel/sector_panel_summary_k6.txt):");
    println(io, "  K*=6 OoS KS median = 75.1%, mean = 66.5 ± 29.2%, fail count = 11/30.");
end

println("\n[done] $OUT_DIR/sector_panel_summary_k3.{csv,txt}");

...
| C3 | **Time-varying transition matrix**: $T_{ij}(t)$ via logistic regression on macro features (VIX, realized vol, term spread) | OPEN (first-pass lagged-RV TVT implemented 2026-04-22) |
...
**C3 first-pass result summary (2026-04-22):** `run_track_c3_time_varying_transition.jl` fits a post-hoc logistic transition model
$P(s_t=j\mid s_{t-1}=i,x_t)=\text{softmax}(a_{ij}+b_{ij}x_t)$ using lagged standardized 20-day realized volatility as the covariate. Outcome: the idea is viable but **does not beat C3a as the headline result**. `CHMM-t TVT` remains clean at 1 % and 5 % VaR (LR_uc 0.01 / 0.50, LR_ind 0.13 / 2.56) but is not materially better than flat `CHMM-t` under C3a (0.10 / 0.01, 0.09 / 0.19). `CHMM-L TVT` improves unconditional coverage materially over flat `CHMM-L`, while `CHMM-N TVT` worsens independence. Interpretation: transition dynamics help some families, but the main paper claim should still center flat `CHMM-t` with Viterbi-conditioned VaR. Full numbers in `results/track_c3/VaR_time_varying_transition_LR_tests.txt`.
